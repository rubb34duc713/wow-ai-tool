...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		const summary = await summariseLLM(transcript, data.prompt);
...
async function summariseLLM(text: string, prompt?: string) {
	...
	try {
		...
		return await callLLM(xai, text, prompt ?? cfg.LLM_PROMPT);
	} catch {
		const openai = new OpenAI({ apiKey: cfg.OPENAI_API_KEY });
		return await callLLM(openai, text, prompt ?? cfg.LLM_PROMPT);
	}
}

async function callLLM(client: OpenAI, text: string, prompt?: string) {
	const { choices } = await client.chat.completions.create({
		model: 'gpt-4o-mini',
		messages: [
			{
				role: 'system',
				content: prompt ?? 'You are a concise crypto-research summariser.'
			},
